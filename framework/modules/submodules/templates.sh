...
#_template=$(echo $(cat "$__DIR__/templates/generic.html"))
#_template=${_template//\{\{user\|upper\}\}/JARVIS}
#_template=${_template//\{\{salutation\|title\}\}/Greetings Friend}
#_template=${_template//\{\{message\}\}/I have succesfully installed Dropbox on stark-industries. Unfortunately Dropbox still requires a human touch to finish the process.}
#_template=${_template//\{\{closing\|capitalize\}\}/Your servant,}
#_template=${_template//\{\{signature\|capitalize\}\}/Jarvis}
#_template=${_template//\{\{user\|lower\}\}/jarvis}
#_template=${_template//\{\{host\|lower\}\}/stark-industries}
...
mw::templates::__internal_format_data() {
    ...
    case "$_action" in
        ...
        lower)
            ...
            ;;
        title)
            for _word in "$_value"; do printf '%s ' "${_word^}"; done
            return 0
            ;;
        upper)
            echo -n "${_value^^}"
